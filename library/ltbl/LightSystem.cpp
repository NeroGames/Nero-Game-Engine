...
namespace ltbl
{
        ...
        , mUseNormals(useNormals)
    {
        ...
        mPenumbraTexture.loadFromMemory(
            priv::penumbraTexture,
            (sizeof(priv::penumbraTexture) / sizeof(*priv::penumbraTexture)));
    ...
    void LightSystem::render(sf::RenderTarget& target, unsigned int lightFactor)
    {
        ...
        sf::FloatRect viewBounds =
            sf::FloatRect(view.getCenter() - view.getSize() * 0.5f, view.getSize());
        ...
        float shadowExtension =
            priv::vectorMagnitude(view.getSize() * -0.5f) * mDirectionEmissionRadiusMultiplier;
        sf::Vector2f extendedBounds = sf::Vector2f(1.f, 1.f) *
                                      std::max(viewBounds.width, viewBounds.height) *
                                      mDirectionEmissionRadiusMultiplier;
        sf::FloatRect extendedViewBounds =
            priv::rectFromBounds(-extendedBounds,
                                 extendedBounds + sf::Vector2f(mDirectionEmissionRange, 0.0f));
    ...
    LightShape* LightSystem::createLightShape(const sf::Sprite& sprite)
    {
        ...
        lightShape->setPoint(
            2u,
            {sprite.getTextureRect().width * 1.f, sprite.getTextureRect().height * 1.f});
    ...
    void LightSystem::update(sf::Vector2u const& size)
    {
        ...
        if(size.x != 0 && size.y != 0)
        {
            ...
            mLightOverShapeShader.setUniform("targetSizeInv",
                                             sf::Glsl::Vec2(1.0f / size.x, 1.0f / size.y));
        }
    }
