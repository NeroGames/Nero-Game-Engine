...
namespace nero
{
    CommandLine::CommandLine()
        : CLI::App("Nero game engine command line interface\nLearn more at "
                   "https://nero-game.com/v2/command-line-interface",
                   "nero")
    {
        // option version
        ...
        auto option1 =
            m_CommandEditorRestart->add_option("--pid", m_ProcessId, "Process to kill and restart");
        option1->required();
        // stop
    ...
    void CommandLine::restartEditor()
    {
        ...
        if(commandEditorRestart)
        {
            ...
            std::string enginePath =
                std::string(getenv("NERO_GAME_HOME")) + "\\Editor\\\"Nero Game Engine.exe\"";
