...
namespace nero
{
    Scene::Context::Context(std::string          sceneName,
    ...
    {
        float32 b2TimeStep = m_LevelSetting->getFloat("time_step") > 0.0f
                                 ? 1.0f / m_LevelSetting->getFloat("time_step")
                                 : float32(0.0f);
        ...
        if(!m_HideWorld)
        {
            ...
            {
                b2TimeStep = 0.0f;
                m_SceneContext.engineType == EngineType::RENDER_ENGINE
                    ? m_InformationContent = StringPool.BLANK
                    : m_InformationContent = "#-- PAUSED --#";
            }
            else
            {
                m_InformationContent = StringPool.BLANK;
            ...
            m_PhysicWorld->Step(b2TimeStep,
                                m_LevelSetting->getInt("velocity_iteration"),
                                m_LevelSetting->getInt("position_iteration"));
    ...
    void Scene::handleMouseButtonInput(const sf::Event::MouseButtonEvent& mouse,
    ...
    void Scene::loadGameLevel(const std::string& levelName)
    {
        ...
        auto gameLevelJson = file::loadJson(
            file::getPath({m_GameSetting->getString("game_level_directory"), levelName}));
    ...
    void Scene::setupLighting()
    {
        ...
        m_LightManager->create({-1000.f,
                                -1000.f,
                                (float)m_SceneContext.renderTexture->getSize().x,
                                (float)m_SceneContext.renderTexture->getSize().y},
                               m_SceneContext.renderTexture->getSize());
