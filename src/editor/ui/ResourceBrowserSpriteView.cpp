...
namespace  nero
{
    ...
    void ResourceBrowserSpriteView::render()
    {
        ...
        auto& spriteTable       = resourceManager->getTextureHolder()->getSpriteTable();
        ...
        for (int i = 0; i < spriteCount; i++)
        {
            ...
            if(ImGui::ImageButton(textureHolder->getSpriteTexture(spriteTable[i]), ImVec2(spriteSize.x, spriteSize.y)))
            {
                if(m_EditorContext->getBuilderMode() == BuilderMode::OBJECT)
                {
                    ...
                    const auto position = m_EditorContext->getNewGameObjectPosition();
                    worldBuilder->addObject(Object::Sprite_Object, spriteTable[i], position);
                }
            }
