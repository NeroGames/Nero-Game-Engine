...
namespace nero
{
    ...
    void OpenProjectTab::render()
    {
        ...
        for(const std::string workspace :
            m_EditorContext->getProjectManager()->getWorkspaceNameTable())
        {
            if(ImGui::CollapsingHeader(workspace.c_str()))
            {
                ...
                for(const nlohmann::json& project :
                    m_EditorContext->getProjectManager()->getWorkspaceProjectTable(workspace))
                {
                    ...
                    {
                        std::string project_lead =
                            ": " + project["project_lead"].get<std::string>();
                        std::string project_company =
                            ": " + project["company_name"].get<std::string>();
                        std::string project_description = project["description"].get<std::string>();
                        std::string project_type =
                            ": " + project["project_type"].get<std::string>();
                        std::string code_editor = ": " + project["code_editor"].get<std::string>();
                        std::string project_namespace =
                            ": " + project["project_namespace"].get<std::string>();
                        std::string creation_date =
                            ": " + project["creation_date"].get<std::string>();
                        std::string modification_date =
                            ": " + project["modification_date"].get<std::string>();
                        ...
                        ImGui::SetCursorPosX(40.f);
                        ImGui::BeginChild(projectName.c_str(),
                                          ImVec2(0.f, 100.f),
                                          true,
                        ...
                        if(ImGui::Button("Open Project", ImVec2(125, 30.f)))
                        {
                            m_EditorContext->getEditorProxy()->openProject(
                                file::getPath({m_EditorContext->getProjectManager()
                                                   ->findWorkspace(workspace)["workspace_directory"]
                                                   .get<std::string>(),
                                               "Project",
                                               projectName}));
